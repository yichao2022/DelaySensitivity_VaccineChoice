...
\hline
 ...
 & ($n = 446$) & ($n = 226$) & ($n = 261$) \\
\hline
Waiting-time coefficient & $-0.274$ & $-0.412$ & $-0.772$ \\
($SE$) & ($0.040$) & ($0.065$) & ($0.079$) \\
Random-coefficient SD & $1.250$ & $1.490$ & $1.801$ \\
MWTA (RMB/month) & 83 & 129 & 250 \\
\hline
\multicolumn{4}{@{}p{\textwidth}@{}}{\footnotesize \textit{Note.} Subgroup-specific mixed logit models with a normally distributed random coefficient on waiting time (400 simulation draws, respondent-level panel). MWTA converts the waiting-time coefficient to RMB/month using the full-sample scaling constants (see Appendix~E). These point estimates are descriptive; the continuous trust interaction was not statistically significant (Appendix F2), so we do not interpret subgroup differences as evidence of systematic trust moderation.}
\end{tabular}
\end{table}
