\section*{Appendix E. Trade-Off Computation}
\label{app:E}
...
Using the mixed logit estimates from the main text ($\beta_{\text{wait}} = -0.297$, $\beta_{\text{cash}} = 0.424$; $\text{SD}_{\text{wait}} = 2.206$ months, $\text{SD}_{\text{cash}} = 311.5$ RMB):

\[
\text{MWTA} = -\frac{-0.297}{0.424} \times \frac{311.5}{2.206} = 0.701 \times 141.2 \approx 99 \text{ RMB/month}
\]

The delta-method 95\% confidence interval is 80--118 RMB/month (delta-method SE: 9.6 RMB/month). The main text reports MWTA only for the full-sample mixed logit. Trust-specific subgroup MWTA estimates are reported in the next section; these are interpreted as exploratory given that the continuous specification test did not support a systematic trust-by-waiting interaction.

\subsection*{E3. Subgroup monetary equivalents}

For completeness, we report the monthly monetary equivalents (MWTA) computed from the subgroup-specific mixed logit estimates in Appendix~F (500 Halton draws, respondent-level panel). The full-sample estimate is 99 RMB/month; subgroup estimates are 59 RMB/month (high trust), 101 RMB/month (low trust), and 205 RMB/month (neutral trust). These point estimates are descriptive: the continuous trust specification did not support systematic moderation (linear interaction $p = 0.15$; quadratic-vs-linear LR $p = 0.27$), and we do not interpret the subgroup MWTA differences as evidence of systematic trust moderation.
