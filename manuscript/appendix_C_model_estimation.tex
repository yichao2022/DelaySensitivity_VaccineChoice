...
\subsection*{C4. Predicted probabilities}

Waiting time was varied from 0 to 6 months and standardized using the estimation-sample mean and standard deviation. Other vaccine attributes were held at their estimation-sample means: continuous standardized attributes (efficacy, cash incentive) and effects-coded side-effect contrasts at zero, and vaccine origin at the sample proportion of domestic-origin alternatives. Because the two vaccine alternatives are identical under this benchmark, the probability of choosing any vaccine is the sum of their probabilities, $P(A) + P(B) = 1 - P(\text{opt-out})$. Probabilities were computed by simulation averaging over the estimated Normal random-coefficient distribution (10{,}000 draws per point). Ninety-five percent confidence intervals were obtained by parametric bootstrap (2{,}000 draws from the asymptotic Normal distribution, 500 within-bootstrap simulations each), using the 2.5th and 97.5th percentiles.

\paragraph{Figure~1 benchmark robustness.} The benchmark profile holds non-waiting attributes at estimation-sample means, which include opt-out structural zeros for efficacy and cash incentive. This may understate baseline vaccination probability. We assess sensitivity via two alternatives: (1)~fixing attributes at vaccine-alternative-only means, yielding probabilities of 80.0\% at 0 months and 55.7\% at 6 months ($\Delta = -24.3$~pp); (2)~average marginal predictions across all choice sets with counterfactual waiting times, yielding 81.3\% to 57.7\% ($\Delta = -23.6$~pp). Both exceed the primary $\Delta = -17.3$~pp, indicating the headline effect is conservative.
\clearpage
